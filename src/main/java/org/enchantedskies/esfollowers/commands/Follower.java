...
public class Follower implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String args[]) {
        ...
        if (playerPetMap.containsKey(player.getUniqueId())) {
            player.sendMessage(ChatColor.RED + "You already have a pet spawned.");
            return true;
        }
        PetArmorStand petArmorStand = new PetArmorStand(plugin, player, player.getLocation(), getPlayerSkull(player.getUniqueId()), getColouredArmour(Material.LEATHER_CHESTPLATE, "#7BC28E"), getColouredArmour(Material.LEATHER_LEGGINGS, "#7BC28E"), getColouredArmour(Material.LEATHER_BOOTS, "#7BC28E"));
        petArmorStand.startMovement(0.4);
        playerPetMap.put(player.getUniqueId(), petArmorStand.getArmorStand().getUniqueId());
        player.sendMessage(ChatColor.GREEN + "Pet Spawned.");
        return true;
    }

//    public void petMovement(ArmorStand armorStand, Player player, double speed) {
//        new BukkitRunnable() {
//            public void run() {
//                if (armorStand == null) {
//                    cancel();
